...
def test_pcd_read_ply():
    ply = pcp.read_ply('data/potato.ply')
    print('\n', ply)

def test_pcd_read_plys():
    ply_merged = pcp.read_plys(['data/weed.plant0.ply', 'data/weed.cleanedbg.ply'])
    ply1 = pcp.read_ply('data/weed.plant0.ply')
    ply2 = pcp.read_ply('data/weed.cleanedbg.ply')
...
def test_pcd_read_ply_unit():
    ...
    for i, u in enumerate(units):
        ply = pcp.read_ply('data/potato.ply', unit=u)
    ...
    for wu in wrong_units:
        with pytest.raises(TypeError) as excinfo:
            ply = pcp.read_ply('data/potato.ply', unit=wu)
            print('\n', excinfo.value)
        assert "as unit, please only tape m, cm, mm, or km." in str(excinfo.value)

def test_pcd_read_planteye_wrong_unit():
    ply = pcp.read_ply('data/down_sample/down_sample_M.ply', unit='m')

def test_pcd_read_planteye_right_unit():
    ply = pcp.read_ply('data/down_sample/down_sample_M.ply', unit='mm')
...
def test_shp_read_shp(shp_config):
    shp = pcp.read_shp(shp_config['p1'])
    print('\n', shp.keys())

def test_shp_read_shps(shp_config):
    shp1 = pcp.read_shp(shp_config['p1'])
    shp2 = pcp.read_shp(shp_config['p2'])
    shp_merge = pcp.read_shps([shp_config['p1'], shp_config['p2']])
    assert len(shp_merge) == len(shp1) + len(shp2)

def test_shp_read_shps_overwrite(shp_config):
    shp_merge = pcp.read_shps([shp_config['p1'], shp_config['p1']], rename=False)

def test_shp_read_shps_correct_coord_one4all(shp_config):
    shp_merge = pcp.read_shps([shp_config['p1'], shp_config['p2']], shp_config['cc'])
