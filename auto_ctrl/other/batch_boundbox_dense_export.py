...
project_filename = '-v051-boundbox8-med'
...
for j in range(folder_count): #run the following code for each folder    
    ...
    for file in file_list: #search file list for .psx
        if (file.rsplit(".",1)[-1].lower() in  ["psx", "psz"]) and ('v051' in file) and ('canon' in file):# and ('group2' in file) and ('sp4' in file): #do stuff if project file
            #open project
            ...
            chunk.exportPoints(path = filename_list[j]+project_filename+'.ply')
            ...
            break#continue #break out of for loop after first .psx file found
